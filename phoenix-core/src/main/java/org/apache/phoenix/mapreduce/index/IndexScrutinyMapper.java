...
public class IndexScrutinyMapper extends Mapper<NullWritable, PhoenixIndexDBWritable, Text, Text> {
    ...
    protected void setup(final Context context) throws IOException, InterruptedException {
        ...
        try {
            ...
            // name)
            targetTableQuery =
                    QueryUtil.constructSelectStatement(qTargetTable, targetColNames, targetPksCsv,
                        Hint.NO_INDEX) + " IN ";
    ...
    private void queryTargetTable(Context context, PreparedStatement targetStatement,
        ...
        while (targetResultSet.next()) {
            ...
            int targetPk = getPkHash(pkObjects);
            // use the pk to fetch the source table column values
            List<Object> sourceValues = targetPkToSourceValues.get(targetPk);
            ...
            if (isIndexedCorrectly) {
                ...
                if (outputInvalidRows) {
                    outputInvalidRow(context, targetValues, sourceValues);
                }
            }
    ...
    private void outputInvalidRow(Context context, List<Object> targetValues,
            List<Object> sourceValues) throws SQLException, IOException, InterruptedException {
        if (OutputFormat.FILE.equals(outputFormat)) {
            context.write(new Text(Arrays.toString(sourceValues.toArray())),
                new Text(Arrays.toString(targetValues.toArray())));
        } else if (OutputFormat.TABLE.equals(outputFormat)) {
            writeToOutputTable(context, sourceValues, targetValues);
        }
    }

    // pass in null targetValues if the target row wasn't found
    private void writeToOutputTable(Context context, List<Object> sourceValues, List<Object> targetValues)
            throws SQLException {
        ...
        outputUpsertStmt.setInt(index++, getPkHash(sourceValues.subList(0, numSourcePkCols))); // SOURCE_ROW_PK_HASH
    ...
    private int getPkHash(List<Object> pkObjects) {
        return Objects.hash(pkObjects.toArray(new Object[pkObjects.size()]));
    }
}
